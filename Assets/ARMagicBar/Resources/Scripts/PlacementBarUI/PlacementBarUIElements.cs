...
namespace ARMagicBar.Resources.Scripts.PlacementBarUI
{
    public class PlacementBarUIElements : MonoBehaviour
    {
        ...
        public void RebuildUI(List<PlacementObjectSO> newObjects)
        {
            // Clear existing UI elements
            foreach (var uiItem in _uiObjectGameObjects)
            {
                if (uiItem != null)
                    Destroy(uiItem.gameObject);
            }

            _uiObjectGameObjects.Clear();
            pageToUIObjects.Clear();

            // ?? Remove old pagination/hide buttons if they exist
            foreach (Transform child in uiObjectParent)
            {
                if (child.name == HIDEOBJECT_NAME || child.name == PAGINATIONOBJECT_NAME)
                {
                    Destroy(child.gameObject);
                }
            }

            // ? Add a single pagination button
            if (enablePage)
            {
                PlacementObjectUiItem paginationUiItem = Instantiate(uiItemPrefab, parent: uiObjectParent);
                paginationUiItem.AddComponent<PaginationUIElement>();
            ...
            int index = 0;
            maxPage = (int)Math.Ceiling((double)newObjects.Count / MAX_ITEMS_PER_PAGE);

            foreach (var obj in newObjects)
            {
                var instantiatedUiItemElement = Instantiate(uiItemPrefab, parent: uiObjectParent);
                instantiatedUiItemElement.SetTexture(obj.uiSprite);
                instantiatedUiItemElement.SetCorrespondingObject(obj.placementObject);
                instantiatedUiItemElement.CorrespondingPlacementObjectSO = obj;
                instantiatedUiItemElement.gameObject.name = obj.placementObject.name;
